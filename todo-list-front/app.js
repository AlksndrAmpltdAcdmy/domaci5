...
document.getElementById('izmjena_zadatka_forma').addEventListener('submit', function(e){
    e.preventDefault();
    let index = parseInt(document.getElementById('index_izmjena').value);
    let izmjenjen_tekst = document.getElementById('izmjena_tekst').value;
    let izmjenjen_opis = document.getElementById('izmjena_opis').value;
    $.ajax({
        type: "POST",
        url: api_route + '/edit_task.php',
        data: {i: index, tekst: izmjenjen_tekst, opis: izmjenjen_opis},
        success: (result) => {
            if(result == "OK"){
                renderuj();
